...
public class UserDBImpl implements IUserDAO {
	...
	public User addAndGetUser(String login, String name, String password, String email) {
		...
		try {
			...
			psFindUser = cn.prepareStatement(SqlQuery.SELECT_FOUND_USER);
			psFindUser.setString(SqlQuery.LOGIN_POSITION, login);
			psFindUser.setString(SqlQuery.PASSWORD_POSITION, password);
			
			psAddUser = cn.prepareStatement(SqlQuery.INSERT_USER);
			...
			synchronized (UserDBImpl.class) {
				if (!psFindUser.executeQuery().next()) {
			...
			DbConnectionManager.closeResourses(rs, cn, psFindUser, psAddUser);
		}
	}
	...
	public User getUser(String login, String password) {
		...
		try {
			cn = DbConnectionManager.createConnection();
			ps = cn.prepareStatement(SqlQuery.SELECT_FOUND_USER);
